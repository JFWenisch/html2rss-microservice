...
public class Controller {
	...
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Related flights found", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = ScrapeResponse.class)) }),
	 ...
	 * @throws Exception
	 */
	public ResponseEntity<ScrapeResponse> process(@RequestBody ScrapeConfig request) throws Exception {
	
		return new ResponseEntity<ScrapeResponse>(ScrapeManager.getInstance().test(request), HttpStatus.OK);
